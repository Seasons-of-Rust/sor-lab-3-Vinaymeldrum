...
impl Shop {
    ...
    fn most_expensive(&self) -> u32 {
        if self.cards[0].price > self.cards[1].price && self.cards[0].price > self.cards[2].price {
            return self.cards[0].price;
        }
        if self.cards[1].price > self.cards[0].price && self.cards[1].price > self.cards[2].price {
            return self.cards[1].price;
        }
        return self.cards[2].price;
    }

    /// Get the total damage of all cards in the shop
    fn total_damage(&self) -> u32 {
        return self.cards[0].damage + self.cards[1].damage + self.cards[2].damage;
    }

    /// Get the total health of all cards in the shop
    fn total_health(&self) -> u32 {
        return self.cards[0].health + self.cards[1].health + self.cards[2].health;
    }
}
